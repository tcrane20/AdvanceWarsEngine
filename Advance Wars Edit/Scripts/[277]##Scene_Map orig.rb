...
=end
class Scene_Map
	attr_accessor :show_ranges, :selected_unit, :phase, :player_turn, :wait, :decided_spot_x, :decided_spot_y, :action, :selected_unit
	attr_reader :build_window
	#--------------------------------------------------------------------------
	# * Main Processing
	#--------------------------------------------------------------------------
	def main
    ...
    $spriteset.update
		...
		@drop_dir_2 = nil
		#-----------------------------------------------------
		# Cursor move speed increase.
		cursor.move_speed = 5
		# Determines whose turn it is by using @army(X) values
    @player_turn = $game_map.army[0]
		$spriteset.update_fow
		# Move cursor to starting position
		cursor.x = @player_turn.x
    cursor.y = @player_turn.y
		# Make message window
    ...
    $target_ranges = []
    for i in 1..264
    #  $move_ranges.push(Sprite_Range.new($spriteset.viewport1, 1, 0, 0,false))
    #  $target_ranges.push(Sprite_Range.new($spriteset.viewport1, 1, 0, 0,false)) if i <= 50
    end
  ...
  def process_movement
	...
	#-----------------------------------------------------------------------------
	def determine_unit_commands(unit)
		commands = [] # Shall push all the commands into this variable
		
		### If the cursor is choosing a spot where a different unit exists
		if !$game_map.get_unit(cursor.x, cursor.y, false).nil? and $game_map.get_unit(cursor.x, cursor.y) != unit
			other_unit = $game_map.get_unit(cursor.x, cursor.y)
			# If the unit is the same army type
			if other_unit.army == unit.army
				####### If units are the same type and one is not at full HP and neither are carrying units
				if other_unit.unit_type == unit.unit_type and other_unit.unit_hp < 10 and unit.holding_units.size == 0 and other_unit.holding_units.size == 0
					@unit_to_join_with = other_unit
					commands.push("Join")
					Config.play_se("decide")
					####### If unit can be loaded into this other unit
				elsif other_unit.carry_capability(unit)
					commands.push("Load")
					@carrying_unit = other_unit
					Config.play_se("decide")
				else
					$game_system.se_play($data_system.buzzer_se)
				end
				# If other unit is not of same army type
			else
				$game_system.se_play($data_system.buzzer_se)
			end
			return commands
      ...
      ranges.flatten.compact.each{|r|
				next unless !r.nil? && valid?(r[0],r[1])
			...
			unit == $game_map.get_unit(cursor.x, cursor.y)
		...
		if unit.can_capture and $game_map.property?(cursor.x, cursor.y)
			building = $game_map.get_tile(cursor.x, cursor.y)
			# If this is a missile silo
			if building.id == TILE_SILO
				commands.push("Launch") unless building.launched
				# If this is a normal property
			else
				commands.push("Capt") if building.army != unit.army
			end
		end
		
		### If unit is holding another unit and can drop it
		if unit.holding_units.size > 0 and unit.valid_drop_spot(cursor.x, cursor.y)
			...
			for u in 0...unit.holding_units.size
				...
				for i in 0...units_nearby.size
					...
					if units_nearby[i].nil? or units_nearby[i] == unit
						if unit.test_drop_tile(tiles_nearby[i], held_unit) == true
		...
		if unit.can_supply
			...
			units_nearby.each{|u|
				...
				# If the unit is of the same army and has low supplies, unless this unit
				# is the one being given orders right now
				if u.army == unit.army and (u.fuel < u.max_fuel or u.ammo < u.max_ammo) and
					u != unit
  ...
  #    end
	...
	#--------------------------------------------------------------------------
	# Modify the @zones array to remove places that do not satisfy the condition.
    ...
    zone = @zones[x][y]
			...
			if !u.nil?
				...
				if u == unit and type == 0
					...
					if !unit.test_drop_tile($game_map.get_tile(zone.x,zone.y), held_unit) or @first_drop_spot == [zone.x,zone.y]
						@zones[x,y] = nil
					end
					#@zones[i] = 0 if @first_drop_spot == [x,y]
					next
				...
				elsif u.army == @player_turn or !unit.can_attack?(u)
					@zones[x,y] = nil
				end
			else
				...
				unless unit.nil?
					...
					# If this location cannot drop a unit here
					if !unit.test_drop_tile(tile, held_unit) or @first_drop_spot == [zone.x,zone.y]
						@zones[x,y] = nil
					end
				end
			end
	...
	#--------------------------------------------------------------------------
	# Returns true if the second held unit can be dropped, even after the first
	# dropped unit has taken a possible spot. If false, the carrying unit will
	# process its drop command for the first unit only (that process is located in
	# phase 4).
	#--------------------------------------------------------------------------
...
=end
  ...
  #--------------------------------------------------------------------------
	...
	#--------------------------------------------------------------------------
	def show_move_ranges(show = true)
	...
	#--------------------------------------------------------------------------
	# If a unit is at this location, return true
	#--------------------------------------------------------------------------
	def occupied?(x, y)
	...
	#-------------------------------------------------------------------------
	# Viewing Ranges - Checks if ranges are being viewed. Needed for graphics.
	#-------------------------------------------------------------------------
	def viewing_ranges
	...
	#-------------------------------------------------------------------------
	# Wait command. Pauses player input and some scene processing.
  # You can also set 'frames' to -1 and it will not decrease. Thus, you could
  # make an infinite pause until some condition is met and set @wait = 0 after.
	#-------------------------------------------------------------------------
	def wait(frames)
		return if @wait > 0
		@wait = frames
